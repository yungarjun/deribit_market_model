...
def _stage0_block(C_train, C_test, train_df, test_df, cfg: Algo1Config, spot_train, spot_test):
...
def run_algo1_deribit(df_raw: pd.DataFrame, cfg: Algo1Config, reference_date, r=0.0, q=0.0) -> Algo1Outputs:
    ...
    # Resample lattice panels to 5-min means, then lightly fill along time
    train_5m = (C_train
                .resample("1T").mean()
    ...
    # Also aggregate the spot (underlying) for reference/diagnostics
    def _spot_5m(d):
        s = (d[['timestamp','underlying_price']]
            .assign(timestamp=pd.to_datetime(d['timestamp']))
            .groupby('timestamp')['underlying_price']
            .median()
            .sort_index())
        return s.resample("1T").mean()

    spot_train_5m = _spot_5m(train_df)
    spot_test_5m  = _spot_5m(test_df)
    ...
    # stage-0
    pca_price, net0, scaler0, sigma_train, sigma_test, gamma_train, gamma_test = _stage0_block(C_train, C_test, train_df, test_df, cfg, spot_train_5m, spot_test_5m)
    ...
    first_stat = pca_stat.explained_variance_ratio_[0]
   ...
   # ----- Static Arbitrage Factors on leftover residual ------
    ...
    R2_train = r2_score(C_train.values[:-1].flatten(), C_hat_train.flatten())
    R2_test  = r2_score(C_test.values[:-1].flatten(),  C_hat_test.flatten())
